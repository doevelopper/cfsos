...
function prepare_rauc_signing() {
    local key="${BR2_EXTERNAL_CFSOS_PATH}/board/raspberrypi3-64/certs/key.pem"
    local cert="${BR2_EXTERNAL_CFSOS_PATH}/board/raspberrypi3-64/certs/cert.pem"

    if [ ! -f "${key}" ]; then
        echo "Generating a self-signed certificate for development"
        "${BR2_EXTERNAL_CFSOS_PATH}"/board/raspberrypi3-64/scripts/generate-signing-key.sh "${cert}" "${key}"
    fi
}
...
function write_rauc_config() {
    ...
    (
        "${HOST_DIR}/bin/tempio" \
            -template "${BR2_EXTERNAL_CFSOS_PATH}/board/raspberrypi3-64/ota/system.conf.gtpl"
...
function install_rauc_certs() {
    ...
    if [ "${DEPLOYMENT}" == "development" ]; then
        ...
        cp "${BR2_EXTERNAL_CFSOS_PATH}/board/raspberrypi3-64/ota/dev-ca.pem" "${TARGET_DIR}/etc/rauc/keyring.pem"
    else
        cp "${BR2_EXTERNAL_CFSOS_PATH}/board/raspberrypi3-64/ota/rel-ca.pem" "${TARGET_DIR}/etc/rauc/keyring.pem"
    fi
    ...
    if ! openssl verify -CAfile "${BR2_EXTERNAL_CFSOS_PATH}/board/raspberrypi3-64/ota/dev-ca.pem" -no-CApath "${cert}"; then
...
function install_bootloader_config() {
    ...
    if [ "${BOOT_SYS}" == "mbr" ]; then
	    ...
	    cp -f "${BR2_EXTERNAL_CFSOS_PATH}/board/common/bootloader/mbr-part.rules" "${TARGET_DIR}/usr/lib/udev/rules.d/"
    fi
}

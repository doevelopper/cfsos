...
$(OOSB)/build-$(TARGET_BOARD): | $(OOSB)/buildroot-$(BUILDROOT_VERSION) .buildroot-patched
...
$(addsuffix -menuconfig,$(TARGET_BOARD)): %-menuconfig: $(OOSB)/build-$(TARGET_BOARD)/.config
	...
	@echo "   Run $(TERM_BOLD)'$(addsuffix -savedefconfig,$(TARGET_BOARD))' $(TERM_RESET) to update."
...
distclean: clean-all
	...
	@rm -rf .buildroot-patched .buildroot-downloaded $(OOSB)
